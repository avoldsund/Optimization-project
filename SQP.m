...
switch problem
    ...
    [l, B] = generate_truss(m, n, ns, v, dist);
...
%     t = 0.5;
    ...
    while convergence(A, q, f_supp) > 1 && iter < 2000 && alpha > 10^-16%% iter < 1000
...
%%
function [l, B] = generate_truss(m, n, ns, v, dist)
...
function plot_structure(A, l, v, force)
...
for j = 1:m
    pts = [v(:,l(j,1))'; v(:,l(j,2))'];
    plot3(pts(:,1), pts(:,2), pts(:,3), 'LineWidth',25*thickness(j));
end
hold on

% for i = 1:n
%     x = [v(1,i); v(1,i) + force((i-1)*3 + 1)];
%     y = [v(2,i); v(2,i) + force((i-1)*3 + 2)];
%     z = [v(3,i); v(3,i) + force((i-1)*3 + 3)];
%     plot3(x,y,z, 'r');
% end
